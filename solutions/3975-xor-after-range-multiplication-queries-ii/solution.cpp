...
class Solution {
    ...
    int xorAfterQueries(vector<int>& nums, vector<vector<int>>& queries) {
        ...
        // Apply larly (few steps per query)
        for (auto &q : large) {
            int l = q[0], r = q[1], k = q[2], v = q[3];
        ...
        // Apply small-k queries using multiples is n.
        for (auto &entry : smallByK) {
            int k = entry.first;
            auto &qs = entry.second;

            // Pre-allocate diff arrays for all residues
            // For residue r, len floor((n-1 - r)/k) + 1 if r < n, else 0
            ...
            // Materialize mfix and apply to nums along each progression
            for (int r = 0; r < k; ++r) {
                int len = len_r[r];
                if (len == 0) continue;
